...
""")
    ...
    # Generate CXML text for LLM view (like Karpathy does)
    cxml_lines = ["<documents>"]
    for index, i in enumerate(rendered, 1):
        cxml_lines.append(f'<document index="{index}">')
        cxml_lines.append(f"<source>{i.rel}</source>")
        cxml_lines.append("<document_content>")
        try:
            text = read_text(i.path)
            cxml_lines.append(text)
        except Exception as e:
            cxml_lines.append(f"Failed to read: {str(e)}")
        cxml_lines.append("</document_content>")
        cxml_lines.append("</document>")
    cxml_lines.append("</documents>")
    cxml_text = "\n".join(cxml_lines)
...
function updateSelection() {{
  ...
  checkboxes.forEach(cb => {{
    ...
    // Note: We don't need file content here anymore - CXML is pre-generated
  }});
  ...
  // Calculate total size (simplified - could be enhanced if needed)
  let totalSize = 0;
  checkboxes.forEach(cb => {{
    const filePath = cb.dataset.file;
    // For now, just count files. Could store sizes in data attributes if needed
  }});
  ...
  // Note: LLM text is now pre-generated in Python, no dynamic update needed
}}
...
"""  # <-- This closes the return f""" statement
